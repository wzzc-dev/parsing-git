...
fn decode_pack(index: &mut Vec<GitObject>, bytes:&mut Vec<u8>) -> io::Result<String> {
  println!("decode_pack, -----------------------------------------");
  let pack = bytes;
  // PACK
  print!(
    "头部信息 12 字节：PACK:{:?}, ",
    std::str::from_utf8(&pack[0..4])
  );
  // 版本号
  print!("版本号:{:?},", &pack[4..8]);
  // 条目的4字节号 n 个条目
...
fn get_object(pack:&mut Vec<u8>,offset: usize,end: usize, type_id:u8){
  if type_id == 1 || type_id == 3 {
    println!("{:?}",decode_reader((&pack[offset..end]).to_vec()));
  } else if type_id == 2 {
    println!("{:}",decode_tree((&pack[offset..end]).to_vec()).unwrap());
  } else {
